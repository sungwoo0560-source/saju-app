...
#
# H1-b(단일화) 중 제3의 음간값 발견 — manse.py menu_current_situation의
# 로컬 _YANGIN(L17477)이 여기와도 다른 값을 썼음: 乙寅 丁巳 己巳 辛亥 癸亥.
# 이 값의 출처도 불명. 아래 값(乙辰 丁未 己未 辛戌 癸丑, main() 배너와 동일)과
# 함께 참고용으로만 남긴다 — get_yangin()은 어느 쪽도 참조하지 않는다.
_EUMIN_MAP_UNUSED = {
    "乙": "辰",
    "丁": "未",
    "己": "未",
...
# 같은 오행을 공유하는 천간끼리 지지를 공유하는 구조(甲·乙 木 → 午,
# 戊·己 土 → 辰 등)를 따른 표. 코드베이스에 5개의 독립 테이블이 있었고
# 그중 4곳(saju_sinsal 본 파일 포함)이 乙→申으로 되어 있었으나, 甲·乙이
# 짝을 이루는 구조상 乙은 甲과 같은 午가 되어야 정합적이다 — 乙→申은
# 癸(申) 행에서 온 전사 오류로 판단해 甲과 동일한 午로 통일한다.
# (시지 12벌 처리 라운드에서 발견: 활성률 32%에 乙 하나만 값이 갈려
# 갈림률 3.68% 발생 — 형 승인 하 이 표로 5곳 전부 교체.)
HONGYEOM_MAP = {
    "甲": "午",
    "乙": "午",
    "丙": "寅",
...
# 복성귀인(福星貴人) 단일 소스 - 가결(訣) 기준.
# 甲丙相邀入虎鄉 / 更逢鼠穴最高强 / 戊猴己未丁宜酉 /
# 乙癸逢牛卯亦藏 / 庚趕馬頭辛帶巳 / 壬騎龍背喜非常
# 복성귀인 판정 지점 전부가 이 상수를 참조한다(HONGYEOM_MAP과 동일 패턴).
BOKSEONG_MAP = {"甲":"寅","乙":"丑","丙":"子","丁":"酉","戊":"申",
                "己":"未","庚":"午","辛":"巳","壬":"辰","癸":"卯"}


# 효신살(梟神煞) 단일 소스 - 일지에 편인(偏印)이 앉은 일주.
# 정인(正印)은 효신이 아니다. 甲->壬(子) 乙->癸(亥) 丙->甲(寅) 丁->乙(卯)
# 戊->丙(午) 己->丁(巳) 庚->戊(辰戌) 辛->己(丑未) 壬->庚(申) 癸->辛(酉).
# 庚辛이 2개인 것은 土가 辰戌丑未 넷이라 생기는 구조이지 비대칭이 아니다.
HYOSIN_SET = {"甲子","乙亥","丙寅","丁卯","戊午","己巳",
              "庚辰","庚戌","辛丑","辛未","壬申","癸酉"}
...
# 천을귀인(天乙貴人) 단일 소스 - 일간마다 지지 2개.
# 甲戊庚->丑未 / 乙己->子申 / 丙丁->亥酉 / 辛->寅午 / 壬癸->卯巳
CHEONEUL_MAP = {"甲":["丑","未"],"乙":["子","申"],"丙":["亥","酉"],"丁":["亥","酉"],
...
def get_oigyeok(pils):
    ...
    # 종왕격
    # N4-b(조사만, 수정 안 함): 조건 역전 상태 — 종왕격은 정통상 일간이 억부
    # 불가능할 만큼 극단적일 때(=극신강)에만 성립하는데, 이 조건은 완전일치
    # sn=="신강(身强)"이라 오히려 "아직 내격(억부 가능)"인 일반 신강만 잡고
    # 정작 종격이 성립해야 할 극신강(極身强)을 놓친다. 올바른 조건은
    # sn=="극신강(極身强)"(또는 "극신강" in sn). 호출부 0건이라 현재 무해.
    ...
    # 종재격
    # N4-b(조사만, 수정 안 함): 종재격도 정통상 극신약(從弱)일 때 성립하는데
    # 완전일치 sn=="신약(身弱)"이라 극신약(極身弱)을 놓친다. 올바른 조건은
    # sn=="극신약(極身弱)"(또는 "극신약" in sn). 호출부 0건이라 현재 무해.
    ...
    # 종관격
    # N4-b(조사만, 수정 안 함): 동일 패턴 — 올바른 조건은 sn=="극신약(極身弱)".
    # 호출부 0건이라 현재 무해.
        ...
        if _sangmun_jj and _jj == _sangmun_jj:
            result.append({
                ...
                "desc": (
                    ...
                    "주변 가까운 사람의 부고 소식을 접하는 일이 생길 수 있습니다. "
                    "몸의 리듬이 흐트러지기 쉬운 시기입니다. 무리한 일정은 줄이고 휴식을 먼저 챙기세요. "
                    "대운·세운에서 상문살이 겹칠 때는 특히 주의가 필요합니다."
                ),
                "remedy": (
                    "처방: 병원 정기 검진을 미루지 마십시오. "
                    "조상 제사와 기제사를 정성껏 지내면 흉기가 완화됩니다. "
                    "민속에서는 장례식장·납골당 방문을 줄이고, 문상 후에는 소금으로 정화한다고 전해집니다."
                ),
            })
            break  # 중복 방지
        ...
        if _gwanjae_targets and _jj in _gwanjae_targets:
            result.append({
                ...
                "desc": (
                    ...
                    "년지를 기준으로 형살(刑殺) 관계에 해당하는 지지가 원국에 있어 "
                    "관재·구설·처벌의 흉기가 발동합니다. "
    ...
    for _sewoon_yrs, _affected_set in _SAMJAE_GROUPS:
        if _sewoon_jj in _sewoon_yrs and _nyon_jj in _affected_set:
            ...
            result.append({
                ...
                "name": "삼재수",
                "icon": "🔥",
                # 삼재수는 "년주" 고정 1개 위치라 str 유지가 의도적 — 소비처가 str/list 양쪽을 방어하므로 list 통일 불필요
                "위치": "년주",
                "pos": "년주",
                "desc": (
...
def get_extra_sinsal(pils):
        ...
        stars.append(
            {
                "name": "백호대살(白虎)",
                "위치": _bh_found,
                "desc": "다치거나 수술할 일이 생기지 않도록 안전을 잘 챙겨야 하는 기운. 그만큼 강한 추진력과 에너지도 타고났습니다",
            }
        )
    ...
    # 홍염살(紅艶煞) — 일간 기준 지지가 원국에 있을 때 (매력·연애운, SSOT: HONGYEOM_MAP)
    if HONGYEOM_MAP.get(ilgan) in all_jjs:
    ...
    # 효신살(梟神煞) - 일지 편인 (SSOT: HYOSIN_SET)
    if ilju_str in HYOSIN_SET:
        stars.append({"name":"효신살(梟神煞)",
                      "desc":"어머니·인성(印星)과 갈등 구조 — 독립심 강하고 자수성가형. 의존 관계 복잡"})

    # 복성귀인(福星貴人) - 일간 기준 지지가 원국에 있을 때 (SSOT: BOKSEONG_MAP)
    if BOKSEONG_MAP.get(ilgan) in all_jjs:
