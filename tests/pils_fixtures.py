...
CASES = {
    ...
    "DST없음_GMT830_1954봄": {
        ...
        # R8-3(2026-09-26) 相 득령 가산으로 변경: 월지 辰(土)이 일간 辛(金)을
        # 생하는 相 관계라 +12.5가 더해져 중화->신강으로, 종합_용신 순서도
        # 억부 구성이 바뀌며 함께 이동(내용은 동일 3원소, 순서만 변경).
        "baseline": {
            "신강신약": "신강",
            "격국명": "正印(정인)格",
            "종합_용신": ["火", "木", "水"],
    ...
    "DST1987_시작전_0510_0030_구간밖": {
        ...
        # R8-3(2026-09-26) 相 득령 가산으로 변경: 월지 巳(火)가 일간 己(土)를
        # 생하는 相 관계라 +12.5가 더해져 중화->신강으로, 종합_용신에 木이
        # 새로 추가됨(억부 구성 변화).
        "baseline": {
            "신강신약": "신강",
            "격국명": "正印(정인)格",
...
TWELVE_BEOL_EXPECT = {
    ...
    "12벌_수렴낮음_19301009": {
        ...
        # R8-3(2026-09-26) 相 득령 가산으로 변경: 12개 시간 후보 중 월지=인성
        # 相 관계였던 일부 후보의 신강신약이 중화->신강으로 넘어가 수렴
        # 후보 집합 자체가 바뀜(내용 불일치, 판정값 변화).
        "신강약": "신약 또는 신강",
