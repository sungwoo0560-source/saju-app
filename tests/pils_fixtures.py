...
CASES = {
    ...
    "근사구간_1955입춘_직전_연주월주불변": {
        # 절입(1955-02-04 23:17 KST) 1분 전 — 아직 갑오년/축월.
        "birth": (1955, 2, 4, 23, 16),
        "gender": "남",
        "longitude": 126.98,
        "use_yaja_time": True,
    ...
    "근사구간_1955입춘_직후_연주월주flip": {
        # 절입 1분 후 — 을미년/인월로 전환. ★입춘은 연주·월주 경계 동시 담당이라
        # 위 케이스와 짝지어 연주까지 바뀌는 걸 실증하는 유일한 픽스처.
        "birth": (1955, 2, 4, 23, 18),
        "gender": "남",
        "longitude": 126.98,
        "use_yaja_time": True,
    ...
    "근사구간_1948입추_직전_월주불변": {
        # 절입(1948-08-08 00:26 KST) 1분 전 — 아직 己未월.
        "birth": (1948, 8, 8, 0, 25),
        "gender": "여",
        "longitude": 126.98,
        "use_yaja_time": True,
        "expect_pillars": ["乙亥", "乙丑", "己未", "戊子"],
        "baseline": {
            "신강신약": "중화",
            "격국명": "偏財(편재)格",
            "종합_용신": ["水", "木", "金"],
    ...
    "근사구간_1948입추_직후_월주flip": {
        # 절입 1분 후 — 庚申월로 전환. 입추는 연주 경계가 아니라 연주(戊子)는 불변,
        # 월주만 갈리는 걸 보여주는 대조 케이스(위 입춘 쌍과 구분).
        "birth": (1948, 8, 8, 0, 27),
        "gender": "여",
        "longitude": 126.98,
        "use_yaja_time": True,
        "expect_pillars": ["乙亥", "乙丑", "庚申", "戊子"],
        "baseline": {
            "신강신약": "중화",
            "격국명": "正官(정관)格",
            "종합_용신": ["土", "火"],
