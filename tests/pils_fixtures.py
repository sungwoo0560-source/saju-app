...
CASES = {
    ...
    "균시차경계_1031": {
        ...
        # R7-2(2026-09-26): 종합_용신 순서 [木,火,水]→[水,木,火] 갱신. 8글자·
        # 격국은 무변경(균시차 로직과 무관) — 이 표본은 극신약(極身弱)인데,
        # 원인은 839acac(get_yongshin의 sn 완전일치 버그 교정 "신약" in sn,
        # 2026-08-28) 이후 극신약도 억부_용신([水,木])을 정상 반영하면서
        # 조립순서가 억부우선([水,木,火])으로 바뀐 것. 이 baseline은 8/20
        # (acdbdfd, 839acac 이전) 도입값이라 갱신이 누락돼 있었다.
        "baseline": {
            "신강신약": "극신약",
            "격국명": "食神(식신)格",
            "종합_용신": ["水", "木", "火"],
    ...
    "근사구간_1955입춘_정규화후경계_직후": {
        ...
        # R7-2(2026-09-26): 격국명 미정격→食神(식신)格 갱신. 8글자·월지(寅)는
        # 무변경 — 원인은 2b9879d(격국 중기·여기 투간 탐색을 next() 단발매치
        # →순회루프로 교정, 2026-09-15)이며 이 커밋이 바로 이 픽스처를 지목해
        # "비겁(중기 丙) 스킵 후 여기 戊 투출 확인→食神格 정상"이라고 명시했다.
        # baseline.json(apptest_33)은 그때 갱신됐으나 이 파일은 누락돼 있었다.
        "baseline": {
            "신강신약": "중화",
            "격국명": "食神(식신)格",
            "종합_용신": ["火", "木"],
    ...
    "1955입춘_연주월주flip_2300": {
        ...
        "use_yaja_time": True,
        "expect_pillars": ["己亥", "丙申", "戊寅", "乙未"],
        # R7-2(2026-09-26): 위 "근사구간_1955입춘_정규화후경계_직후"와 같은
        # 명식(戊寅월/乙未년) — 격국명 미정격→食神(식신)格 갱신, 원인도 동일
        # (2b9879d, 2026-09-15). 상세 근거는 위 픽스처 주석 참고.
        "baseline": {
            "신강신약": "중화",
            "격국명": "食神(식신)格",
            "종합_용신": ["火", "木"],
    ...
    # ── render_manse_grid 시간모름 회귀(R6-2, 2026-09-24) ──────────────────
    # 위 "박후규_1969_계사일_시간미상"과 birth 동일(이미 검증된 값 재사용) —
    # unknown_time 플래그만 다르므로 case_name을 새로 분리했다(기존 59케이스
    # 골든 0건 변경 원칙 — 기존 케이스명에 플래그를 얹으면 그 케이스의
    # manse_grid 행이 "신규"가 아니라 "기존값 변경"이 되어 원칙 위반).
    # apptest_33._call_menu의 manse_grid 분기만 이 플래그를 읽어 pils[0]을
    # 블랭크 처리한다 — 다른 17메뉴는 이 케이스도 시간확정처럼 그대로 돈다
    # (해는 없으나 계산 중복 — R6-2 진단 보고 참고).
    "박후규_시간모름_grid": {
        "birth": (1969, 11, 14, 12, 0),
        "gender": "남",
        "longitude": 126.98,
    ...
    # ── 곡각살 임계 회귀 픽스처 ────────────────────────────────
    # 곡각 글자(乙己巳丑)가 정확히 3자(丑·巳·乙) — GOKGAK_TH_BALDONG=3의
    # 경계 바로 위. 임계가 4자로 밀리면 '발동'->'참고'로 떨어져 즉시 잡힌다.
    # 4eaf1b3(곡각 SSOT + 글자수 경중 분기)의 회귀 기준선.
    "곡각3자_19550225": {
        "birth": (1955, 2, 25, 3, 0),
        "gender": "남",
        "longitude": 126.98,
        "use_yaja_time": True,
        "expect_pillars": ["辛丑", "丁巳", "戊寅", "乙未"],
        "baseline": {
            "신강신약": "신강",
            "격국명": "傷官(상관)格",
            "종합_용신": ["水", "金", "火", "木"],
    ...
    # ── 바람기 최고등급 픽스처 ────────────────────────────────
    # 바람기 85점(81+ 최고등급)인데 발동 이유가 양일간+편재/홍염/일지충+양인/
    # 식상태왕으로, 도화가 전혀 없이 최고등급에 도달하는 경로. d5912b8에서
    # '外桃花 강력 발동' 단정을 걷어낸 근거가 이 구조다 — 그 문구가 되살아나면
    # 이 케이스에서 잡힌다. 곡각은 2자라 '참고' 등급(발동 경계 아래쪽)도 겸함.
    "바람기81_19870515": {
        "birth": (1987, 5, 15, 9, 0),
        "gender": "여",
        "longitude": 126.98,
        "use_yaja_time": True,
        "expect_pillars": ["戊辰", "甲子", "乙巳", "丁卯"],
        "baseline": {
            "신강신약": "중화",
            "격국명": "偏財(편재)格",
            "종합_용신": ["水", "金"],
    ...
    # ★2026-09-20 절입일 당일 0시대 출생 픽스처 — saju_engine.py:1308
    # "hour = int(hour) if hour else 12"(0이 falsy라 정오로 치환되던 버그,
    # 형 승인 후 수정) 회귀 감지용.
    # 출생: 1979-11-08 00:31(남). 해당 절기=입동, KASI 원값(월,일,시,분)=
    # (11,8,9,32) — 즉 생일 당일이 정확히 절입일이라 0시/12시 차이가 대운수
    # 반올림 경계를 크게 넘나든다. 방향=역행(년간 己=음간, 남명).
    # 수정 전(버그, hour=0→12 취급) 대운: 癸酉(시작나이=1, 시작연도=1980).
    # 수정 후(정답, 분 단위 절입 거리 그대로 사용) 대운: 癸酉(시작나이=9,
    # 시작연도=1988) — 독립 재계산(별도 스크립트, 엔진 무수정)과 100% 일치.
    # 대운수 자체는 baseline({} 스키마)에 없는 항목이라 여기서 직접 assert
    # 하지 않는다 — apptest_33(56케이스 스윕, 대운을 노출하는 lifeline/
    # current_situation 등 메뉴, baseline.json)이 시작나이=1 vs 9 차이를
    # 텍스트 회귀로 잡아준다(1308이 되돌려지면 이 케이스만 DIFF로 걸림,
    # 실측 확인 완료).
    "절입일0시_대운수급변_19791108": {
        "birth": (1979, 11, 8, 0, 31),
        "gender": "남",
        "longitude": 126.98,
        "use_yaja_time": True,
        "expect_pillars": ["甲子", "己卯", "甲戌", "己未"],
        "baseline": {
            "신강신약": "신강",
            "격국명": "月劫格",
            "종합_용신": ["木", "水", "火"],
