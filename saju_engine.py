...
class AstroEngine:
    ...
    def get_solar_term_precision(year, month, day, term_name):
        ...
        # ★이 값들은 kasi_24terms.json의 2000년 데이터 사본이다.
        # JSON을 고치면 여기(ref_times/ref_times_all)도 반드시 함께 고칠 것
        # (f81b288에서 JSON만 고치고 이 사본 갱신이 누락돼 입춘이 4시간 틀린 채
        # 1940-1999·2028-2040 전 구간에 전파된 전례가 있음).


        # 짝수 절기(중기) 포함

        # ★ref_times_all도 kasi_24terms.json의 2000년 데이터 사본 — 위 주석과 동일 원칙
        ref_times_all = {
            "소한": (1, 6, 10, 1),
            "대한": (1, 21, 3, 23),
            "입춘": (2, 4, 21, 40),
...
class TimeCorrection:
    ...
    REGION_LONGITUDE = {
        ...
        "성남": 127.14, "안산": 126.82, "안양": 126.96, "평택": 127.11,
        "의정부": 127.04, "파주": 126.78, "김포": 126.71, "화성": 126.83,
        "남양주": 127.21, "시흥": 126.80, "하남": 127.21,
        # 강원도
        "강원": 127.73,
        "춘천": 127.74, "원주": 127.94, "강릉": 128.88, "속초": 128.59,
        "동해": 129.12, "삼척": 129.17, "태백": 128.99, "홍천": 127.89,
        "양구": 127.99, "인제": 128.17, "고성(강원)": 128.47,
        # 충청북도
        "충북": 127.47,
        "청주": 127.47, "충주": 127.93, "제천": 128.21,
        "진천": 127.43, "음성": 127.69, "영동": 127.78,
        # 충청남도
        "충남": 126.66,
        "천안": 127.15, "공주": 127.11, "아산": 127.01, "서산": 126.45,
        "보령": 126.62, "홍성": 126.66, "당진": 126.63, "논산": 127.10,
        # 전라북도
        "전북": 127.15,
        "전주": 127.15, "군산": 126.72, "익산": 126.96, "남원": 127.39,
        "정읍": 126.85, "김제": 126.88, "완주": 127.16,
        # 전라남도
        "전남": 126.46,
        "목포": 126.39, "여수": 127.66, "순천": 127.49, "광양": 127.69,
        "나주": 126.72, "무안": 126.46, "해남": 126.60, "고흥": 127.28,
        # 경상북도
        "경북": 128.73,
        "포항": 129.36, "경주": 129.22, "구미": 128.34, "안동": 128.73,
        "영주": 128.62, "상주": 128.16, "문경": 128.19, "경산": 128.74,
        "김천": 128.12, "칠곡": 128.40,
        # 경상남도
        "경남": 128.68,
        "창원": 128.68, "진주": 128.11, "통영": 128.42, "거제": 128.62,
        "밀양": 128.75, "거창": 127.91, "합천": 128.17, "고성(경남)": 128.32,
        "사천": 128.07, "양산": 129.04, "함안": 128.41,
        # 제주도
        "제주": 126.53, "제주도": 126.53, "서귀포": 126.56,
    }
...
def calc_ohaeng_strength(ilgan, pils):
    ...
    # ── ⑤ 월령 득령 — 旺·相 (최대 25pt) ────────────────────────────
    # R8-3(2026-09-26, 형 승인): 왕상휴수사(旺相休囚死) 중 旺(당령자왕,
    # 월지=일간 오행)뿐 아니라 相(令生者相, 월령이 생하는 자 — 즉 월지가
    # 일간을 생하면 일간이 相)도 득령의 일종으로 반영한다. 旺은 원래 가산폭
    # (+25) 그대로, 相은 그 절반(+12.5)만 가산 — 相이 旺보다는 약한 형태의
    # 득령이라는 원칙을 수치로 구현. 2,000표본 시뮬(사전 보고) 기준 등급
    # 변동 4.55%·종합_용신 변동 3.95% 규모.
    wol_jj = pils[2]["jj"]
    wol_oh = OH.get(wol_jj, "")
    ilgan_oh = OH.get(ilgan, "")
    _BIRTH_R_5 = {"木": "水", "火": "木", "土": "火", "金": "土", "水": "金"}  # 일간을 생하는 오행(인성)

    if wol_oh == ilgan_oh:
        power[wol_oh] += 25.0
    elif wol_oh == _BIRTH_R_5.get(ilgan_oh, ""):
        power[wol_oh] += 12.5
